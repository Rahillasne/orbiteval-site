...
(() => {
  ...
  function trendSVG(points, threshold, unit) {
    ...
    ticks.forEach((t) => { s += `<line x1="${pl}" x2="${W - pr}" y1="${y(t)}" y2="${y(t)}" style="stroke:var(--line)" stroke-width="1"/><text x="${pl - 8}" y="${y(t) + 4}" text-anchor="end" font-size="11" style="fill:var(--ink-mute)" font-family="IBM Plex Mono, monospace">${Math.round(t * 10) / 10}</text>`; });
    if (threshold != null) s += `<line x1="${pl}" x2="${W - pr}" y1="${y(threshold)}" y2="${y(threshold)}" style="stroke:var(--wait)" stroke-width="1"/><text x="${W - pr}" y="${y(threshold) - 6}" text-anchor="end" font-size="11" style="fill:var(--ink-soft)" font-family="Figtree, sans-serif">threshold ${threshold.toFixed(1)}</text>`;
    if (points.length > 1) s += `<path d="${points.map((p, i) => `${i ? "L" : "M"}${x(i)} ${y(p.value)}`).join(" ")}" fill="none" style="stroke:var(--brand)" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>`;
    points.forEach((p, i) => { s += `<circle cx="${x(i)}" cy="${y(p.value)}" r="4.5" style="fill:var(--brand);stroke:var(--paper-2)" stroke-width="2"/><text x="${x(i)}" y="${H - 12}" text-anchor="middle" font-size="11" style="fill:var(--ink-mute)" font-family="Figtree, sans-serif">${esc(p.label)}</text>`; });
    const lp = points[points.length - 1]; s += `<text x="${x(points.length - 1)}" y="${y(lp.value) - 12}" text-anchor="${points.length > 1 ? "end" : "middle"}" font-size="12" style="fill:var(--ink-2)" font-family="Figtree, sans-serif">${lp.value.toFixed(1)}%</text>`;
